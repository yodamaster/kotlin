...
class Kapt3CommandLineProcessor : CommandLineProcessor {
    ...
    override val pluginOptions: Collection<CliOption> =
            listOf(GENERATED_OUTPUT_DIR_OPTION, ANNOTATION_PROCESSOR_CLASSPATH_OPTION, APT_OPTIONS_OPTION,
                   CLASS_FILES_OUTPUT_DIR_OPTION, VERBOSE_MODE_OPTION, STUBS_OUTPUT_DIR_OPTION)
...
class Kapt3ComponentRegistrar : ComponentRegistrar {
    override fun registerProjectComponents(project: MockProject, configuration: CompilerConfiguration) {
        val sourcesOutputDir = configuration.get(Kapt3ConfigurationKeys.GENERATED_OUTPUT_DIR)?.let(::File) ?: return
        val classFilesOutputDir = configuration.get(Kapt3ConfigurationKeys.CLASS_FILES_OUTPUT_DIR)?.let(::File) ?: return
