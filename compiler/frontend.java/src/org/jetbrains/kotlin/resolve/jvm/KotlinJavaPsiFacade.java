...
public class KotlinJavaPsiFacade {
    ...
    public PsiClass findClass(@NotNull ClassId classId, @NotNull GlobalSearchScope scope) {
        ...
        for (KotlinPsiElementFinderWrapper finder : finders()) {
            if (finder instanceof KotlinCliPsiElementFinder) {
                PsiClass aClass = ((KotlinCliPsiElementFinder) finder).findClass(classId, scope);
    ...
    public Set<String> knownClassNamesInPackage(@NotNull FqName packageFqName) {
        ...
        if (finders.length == 1 && finders[0] instanceof KotlinCliPsiElementFinder) {
            return ((KotlinCliPsiElementFinder) finders[0]).knownClassNamesInPackage(packageFqName);
        }
    ...
    private KotlinPsiElementFinderWrapper[] calcFinders() {
        ...
        KotlinPsiElementFinderWrapper kotlinFinder =
                javaFileManager instanceof KotlinCliJavaFileManager
                ? new KotlinCliPsiElementFinder(javaFileManager)
                : new KotlinPsiElementFinderImpl(project, javaFileManager);
        elementFinders.add(kotlinFinder);
    ...
    private static class KotlinCliPsiElementFinder implements KotlinPsiElementFinderWrapper, DumbAware {
        private final JavaFileManager javaFileManager;

        public KotlinCliPsiElementFinder(@NotNull JavaFileManager javaFileManager) {
    ...
    private static class KotlinPsiElementFinderImpl implements KotlinPsiElementFinderWrapper, DumbAware {
        private final JavaFileManager javaFileManager;
        private final PsiManager psiManager;
        private final PackageIndex packageIndex;

        public KotlinPsiElementFinderImpl(@NotNull Project project, @NotNull JavaFileManager javaFileManager) {
            this.javaFileManager = javaFileManager;
