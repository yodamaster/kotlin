...
fun box(): String {
    ...
    if (!parameters[0].isSynthetic() || parameters[0].isIntrinsic()) return "wrong name flags: ${parameters[0].modifiers}"

    if (parameters[1].name != "\$enum\$ordinal") return "wrong ordinal name: ${parameters[1].name}"
    if (!parameters[1].isSynthetic() || parameters[1].isIntrinsic()) return "wrong ordinal flags: ${parameters[1].modifiers}"
